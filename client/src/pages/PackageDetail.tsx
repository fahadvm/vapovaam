...
import { ArrowLeft, Share2, Heart, MapPin, Calendar, CheckCircle2, MessageCircle } from 'lucide-react';
...
export const PackageDetail: React.FC = () => {
  ...
  if (!pkg) {
    return <div className="text-white p-4">Package not found</div>;
  }
    ...
    <MobileLayout>
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-zinc-800/50 p-4 rounded-xl border border-white/5">
                      ...
                      <div className="font-medium text-white">Oct - Mar</div>
                    </div>
                    <div className="bg-zinc-800/50 p-4 rounded-xl border border-white/5">
                      <CheckCircle2 className="text-green-400 mb-2" size={20} />
                      <div className="text-xs text-zinc-500">Inclusions</div>
                      <div className="font-medium text-white">Flight + Hotel</div>
                    </div>
                  </div>
                ...
                <div className="space-y-6 relative pl-4 border-l border-zinc-800 ml-2">
                  {[1, 2, 3, 4, 5].map((day) => (
                    <div key={day} className="relative pl-6">
                      ...
                      <div className="text-xs font-bold text-blue-400 mb-1">Day {day}</div>
                      <h4 className="text-white font-bold mb-1">Explore the City</h4>
                      <p className="text-sm text-zinc-400">Morning city tour followed by local cuisine tasting.</p>
                    </div>
                  ))}
                </div>
              )}
              ...
              {activeTab === 'Stay' && (
                <div className="space-y-4">
                  <div className="aspect-video rounded-xl bg-zinc-800 overflow-hidden relative">
                    <img src="https://images.unsplash.com/photo-1566073771259-6a8506099945?auto=format&fit=crop&q=80&w=1000" alt="Hotel" className="w-full h-full object-cover" />
                    <div className="absolute bottom-0 left-0 right-0 p-3 bg-gradient-to-t from-black/80 to-transparent">
                      <div className="text-white font-bold">Luxury Resort & Spa</div>
                      <div className="text-xs text-zinc-300">5 Star • Breakfast Included</div>
                    </div>
                  </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4 bg-zinc-900 border-t border-white/10 z-30">
        <div className="flex gap-3">
          ...
          {/* <button className="flex-[2] bg-white hover:bg-zinc-200 text-black font-bold py-3.5 rounded-xl flex items-center justify-center gap-2 transition-colors active:scale-[0.98]">
            Book Now <span className="text-xs font-normal opacity-60">(${pkg.price})</span>
          </button> */}
        </div>
      </div>
    </MobileLayout>
